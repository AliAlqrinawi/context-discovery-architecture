...
| Ancestry is walked for **verification only** — a citation, never a slice ([ADR-A010](decisions/ADR-A010-inheritance-and-annotation-traversal.md) D2 addendum, 2026-09-24). The fetch read of a project ancestor's member body remains an under-build; S1's flag says where the member is and that its body was not fetched | A key-first experiment whose key marks an S1 citation `wrong-slice` — that is, a reviewer who needed the body, not the location. §4 of ADR-A010 forbids the fetch regardless; a finding would reopen that section, not this row |
