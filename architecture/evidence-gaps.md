...
| Hand-written answer keys have now missed a **real defect twice out of two** scored milestones — M17's blind reviewers found a PHP version constraint mismatch in `composer.lock`, M19's found that `CacheGroup`'s key index is never TTL-refreshed so `flush()` eventually forgets nothing. Both verified against the source; neither key was changed | A second pair of eyes on any future key **before** scoring. This is a finding about the method the whole programme rests on (ADR-001), not about either milestone |
