...
| Hand-written answer keys have now missed a real problem **three times out of three** scored milestones — M17 and M20 both surfaced the same unkeyed `composer.json` PHP-constraint mismatch, M19 surfaced `CacheGroup`'s un-refreshed index TTL. All verified against the source; no key was changed | A second pair of eyes on any future key **before** scoring. This is now the most consistent finding of the whole scored series, and it is about ADR-001's method rather than about any milestone |
