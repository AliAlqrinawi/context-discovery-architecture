...
| 7 | [decisions/](decisions/) | ADR-A001…A029 — the architecture decisions, with evidence and rejected alternatives |
...
a hop and is wrong for the own-trait case; ADR-A028's wording narrowed), home
`NamedReferenceAssertionExtractor`, the `imports === []` gate not applying to it. `extends Name`
is **not** a prerequisite — the walk reads it as a fact — and would have admitted 24 parent
assertions, fourteen of them anonymous migration classes; ADR-A027 and ADR-A028 corrected by
appended note. Shape 5 is indistinguishable at the table (every D1 test helper's parent is
`Tests\TestCase`, project code), which turns the S1/S2 split from a preference into an argument.
Shape 6 stays out. **Three changes must land as one**: the form, the premises and D2 walk, and a
guard so ADR-A020's surface fallback never targets the origin file — without it the chain is
Fetched → not found → false flag → the changed file's own surface → the diff duplicated
(ADR-A018/A019). The ADR-A020 guard is the next gate. The M26 golden would go stale while its
test stays green — the worst kind of pass; a new golden and a `POLICY` bump are required.
