...
| **One hunk takes a `return` out of one member and puts one into the next** (M28) | **Nothing.** Every recognised removed return must have left the member the added one belongs to; the removed line's member is read from the position the parser now records, since the current tree does not hold it | Before M28 this produced *"the body of beta now returns a single value or null where it returned a collection"* about a beta that never returned a collection — a false claim, reproduced with a real `git diff`. The rule is "every", so a hunk that changes one member's return **and** removes another's also says nothing: over-rejection in the safe direction. This is the one row in this table that removed a claim rather than accepting a silence, and it was approved as a content change on the ground that the claim it removes is false |
